...
# acronyms handling
filename = 'promptacronymns.txt'
with open(filename, 'r') as file:
    acronymdict = json.load(file)
...
def prune_metadata_str(
    ...
    emb_tuple = sup.load_embeddings("/home/ubuntu/public_embeddings.pkl")
    emb = emb_tuple[0]
...
def get_metadata_str(question):
    for key, value in acronymdict.items():
        pattern = re.compile(r'\b' + key + r'\b', re.IGNORECASE)
        question = pattern.sub(value, question)
    print(question)
    prunedmeta=prune_metadata_str( question, "ttech", True, 55, False)
    return prunedmeta.replace("`","")
